...
// SPI写读一字节函数
uint8_t SPI_WriteReadBytes(SPI_TypeDef *SPIx, uint8_t TxData)
{
	while (!LL_SPI_IsActiveFlag_TXP(SPIx)) { } // 等待可写

	LL_SPI_TransmitData8(SPI1, TxData); // 写入一字节数据

	while (!LL_SPI_IsActiveFlag_RXP(SPIx)){ } // 等待可读

	return LL_SPI_ReceiveData8(SPIx); // 读取一字节数据
}
...
/* Configure the peripherals common clocks */
  ...
  LL_SPI_StartMasterTransfer(SPI1); // 开启主机传输模式
